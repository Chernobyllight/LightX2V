...
class T5OffloadBlocksWeights(WeightModule):
    def __init__(self, block_nums, mm_type, lazy_load=False, lazy_load_path=None, rms_norm_type="sgl-kernel"):
        super().__init__()
        self.block_nums = block_nums
        self.offload_block_buffers = WeightModuleList(
            [T5OffloadSelfAttention(i, mm_type, create_cuda_buffer=True, create_cpu_buffer=False, lazy_load=lazy_load, lazy_load_path=lazy_load_path, rms_norm_type=rms_norm_type) for i in range(1)]
        )
        if lazy_load:
            self.offload_block_cpu_buffers = WeightModuleList(
                [
                    T5OffloadSelfAttention(i, mm_type, create_cuda_buffer=False, create_cpu_buffer=True, lazy_load=lazy_load, lazy_load_path=lazy_load_path, rms_norm_type=rms_norm_type)
                    for i in range(1)
                ]
            )
        ...
        self.blocks = WeightModuleList(
            [
                T5OffloadSelfAttention(i, mm_type, create_cpu_buffer=False, create_cuda_buffer=False, lazy_load=lazy_load, lazy_load_path=lazy_load_path, rms_norm_type=rms_norm_type)
...
class T5OffloadSelfAttention(WeightModule):
    def __init__(self, block_index, mm_type, block_prefix="blocks", create_cuda_buffer=False, create_cpu_buffer=False, lazy_load=False, lazy_load_path=None, rms_norm_type="sgl-kernel"):
        super().__init__()
        ...
        self.add_module(
            "norm1",
            RMS_WEIGHT_REGISTER[rms_norm_type](f"{block_prefix}.{self.block_index}.norm1.weight", create_cuda_buffer, create_cpu_buffer, lazy_load, lazy_load_path),
        )
        self.add_module(
            "norm2",
            RMS_WEIGHT_REGISTER[rms_norm_type](f"{block_prefix}.{self.block_index}.norm2.weight", create_cuda_buffer, create_cpu_buffer, lazy_load, lazy_load_path),
        )
        self.add_module(
            "pos_embedding",
...
class T5Encoder(nn.Module):
    def __init__(
        ...
        if cpu_offload:
            ...
            self.blocks_weights = T5OffloadBlocksWeights(num_layers, quant_scheme, lazy_load, lazy_load_path, rms_norm_type=rms_norm_type)
