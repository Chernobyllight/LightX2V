...
class BaseTransformerModel(ABC):
    ...
    def _init_weights_impl(self, weight_dict=None):
        ...
        try:
            ...
            if shared_owner is not None:
                self._validate_shared_cpu_weights()
        except BaseException:
            if shared_owner is not None:
                try:
                    self.close_shared_cpu_weights()
                except Exception as cleanup_error:
                    logger.error(f"Failed to close shared CPU weight arena after model initialization error: {cleanup_error}")
            raise
    ...
    def _validate_shared_cpu_weights(self):
        """Optional model-specific post-load validation for adopted views."""
