...
class HunyuanImage3TransformerInfer(BaseTransformerInfer):
    ...
    def infer_attention(
        ...
        if self.global_num_heads % active_tp_size or self.global_num_key_value_heads % active_tp_size:
            raise ValueError(
                "HunyuanImage3 active TP size must divide Q and KV heads: "
                f"Q={self.global_num_heads}, KV={self.global_num_key_value_heads}, active_tp_size={active_tp_size}."
            )
    ...
    def _infer_mlp_flashinfer(self, moe, hidden_states):
        ...
        if not hasattr(moe, "ensure_flashinfer_weights") and not hasattr(moe, "active_flashinfer_weight_shards"):
        ...
        for _, canonical_tp_rank, moe_weight, moe_weight_2 in shards:
            if canonical_tp_rank < 0 or canonical_tp_rank >= micro_tp_size:
                raise ValueError(
                    f"HunyuanImage3 FlashInfer canonical TP rank {canonical_tp_rank} is outside micro TP size {micro_tp_size}."
                )
