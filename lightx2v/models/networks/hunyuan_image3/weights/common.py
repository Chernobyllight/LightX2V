...
class HunyuanImage3MoEWeights(WeightModule):
    ...
    def ensure_flashinfer_weights(self, device, dtype):
        ...
        if first_gate_up.ndim != 2 or first_gate_up.shape[0] % (2 * self.micro_shard_count):
            raise RuntimeError(
                f"Invalid HunyuanImage3 FlashInfer gate/up resident shape {tuple(first_gate_up.shape)} for {self.micro_shard_count} micro shards."
            )
        ...
        if first_down.shape != (hidden_size, self.micro_shard_count * micro_intermediate):
            raise RuntimeError(
                "HunyuanImage3 FlashInfer down weight does not match gate/up micro shards: "
                f"gate_up={tuple(first_gate_up.shape)}, down={tuple(first_down.shape)}."
            )

        # Allocate the final resident pack once, then copy and release one
        # expert at a time.  This avoids retaining a second list of all expert
        # tensors in addition to the final pack.
        ...
        for expert_index, expert in enumerate(self.experts):
            ...
            if gate_up.shape != first_gate_up.shape or down.shape != first_down.shape:
                raise RuntimeError(
                    f"HunyuanImage3 expert {expert_index} shape differs while building the universal FlashInfer pack: "
    ...
    def _active_flashinfer_micro_shard_ids(self):
        ...
        if active_tp_size == self.flashinfer_logical_tp_size:
            ...
            return (micro_id,)
        raise RuntimeError(
            f"HunyuanImage3 FlashInfer active TP size must be {self.storage_tp_size} or {self.flashinfer_logical_tp_size}; got {active_tp_size}."
        )
