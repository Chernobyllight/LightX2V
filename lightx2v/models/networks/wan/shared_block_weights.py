...
from lightx2v.common.offload.shared_weight_coordinator import SharedArenaAllocation, materialize_shared_weight_arena
...
def _checkpoint_content_digest(path: Path) -> str:
    for root in path.parents:
        relative_path = path.relative_to(root)
        metadata_path = root / ".cache" / "huggingface" / "download" / relative_path
        metadata_path = metadata_path.with_name(f"{metadata_path.name}.metadata")
        if not metadata_path.is_file():
            continue

        with metadata_path.open(encoding="utf-8") as metadata_file:
            metadata_file.readline()
            digest = metadata_file.readline().rstrip("\r\n")
            timestamp_text = metadata_file.readline().rstrip("\r\n")
        if re.fullmatch(r"[0-9a-f]{64}", digest) is None:
            raise ValueError(f"Invalid SHA-256 on line 2 of Hugging Face metadata file {metadata_path}")
        try:
            metadata_second = int(float(timestamp_text))
...
class WanFp8VllmSharedBlockAdapter:
    ...
    def _validate_config(self, lora_path) -> None:
        ...
        if not self.config.get("dit_quantized", False) or self.config.get("dit_quant_scheme") != "fp8-vllm":
            errors.append("the first Wan adapter requires dit_quantized=true and dit_quant_scheme='fp8-vllm'")
        if self.config.get("tensor_parallel", False):
            errors.append("tensor_parallel is not supported yet")
        ...
        if self.config.get("shared_cpu_weight_backend", "sysv") != "sysv":
            errors.append("shared_cpu_weight_backend must be 'sysv'")
        if self.config.get("shared_cpu_weight_scope", "auto") not in {"auto", "host", "numa"}:
            errors.append("shared_cpu_weight_scope must be 'auto', 'host', or 'numa'")
        if type(self.config.get("shared_cpu_weight_strict_numa", True)) is not bool:
            errors.append("shared_cpu_weight_strict_numa must be a bool")
        chunk_mb = self.config.get("shared_cpu_weight_register_chunk_mb", 128)
        if type(chunk_mb) is not int or chunk_mb <= 0:
            errors.append("shared_cpu_weight_register_chunk_mb must be a positive integer")
        ...
        if self.infer_dtype != GET_SENSITIVE_DTYPE():
            errors.append("the first Wan adapter requires DTYPE and SENSITIVE_LAYER_DTYPE to match")
