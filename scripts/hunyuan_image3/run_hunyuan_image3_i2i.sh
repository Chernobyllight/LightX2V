...
export FLASHINFER_AUTOTUNE_MODE="${FLASHINFER_AUTOTUNE_MODE:-${flashinfer_autotune_mode:-load}}"
...
export HUNYUAN_CFG_MODE="${HUNYUAN_CFG_MODE:-${hunyuan_cfg_mode:-parallel}}"
export CFG_PARALLEL_SIZE="${CFG_PARALLEL_SIZE:-${cfg_parallel_size:-2}}"
export TORCHRUN_LOG_DIR="${TORCHRUN_LOG_DIR:-${lightx2v_path}/save_results/torchrun_logs/hunyuan_image3_i2i}"
...
if os.environ.get("HUNYUAN_CFG_MODE", "parallel").lower() == "parallel":
    parallel = dict(config.get("parallel") or {})
    parallel["cfg_p_size"] = env_int("CFG_PARALLEL_SIZE", 2)
