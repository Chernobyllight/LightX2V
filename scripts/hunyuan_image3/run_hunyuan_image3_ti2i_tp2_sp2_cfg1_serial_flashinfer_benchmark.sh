...
usage() {
    cat <<'EOF'
Usage: run_hunyuan_image3_ti2i_tp2_sp2_cfg1_serial_flashinfer_benchmark.sh [options]
...
if [[ ${#visible_gpus[@]} -ne 4 ]]; then
    echo "Error: TP2+SP2 benchmark requires exactly 4 visible GPUs; got '${CUDA_VISIBLE_DEVICES}'." >&2
    exit 2
fi
