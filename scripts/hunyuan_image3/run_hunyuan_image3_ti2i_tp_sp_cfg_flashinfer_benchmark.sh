...
usage() {
    cat <<'EOF'
Usage: run_hunyuan_image3_ti2i_tp_sp_cfg_flashinfer_benchmark.sh [options]

Topology: 4 GPUs; TP2+SP2; CFG runs serially (no CFG parallel group).
...
benchmark_name="hunyuan_image3_ti2i_tp2_sp2_cfg1_serial_gpu4_flashinfer_benchmark"
...
if [[ ${#visible_gpus[@]} -ne 4 ]]; then
    echo "Error: TP2+SP2 with serial CFG benchmark requires exactly 4 visible GPUs; got '${CUDA_VISIBLE_DEVICES}'." >&2
    exit 2
fi
...
"${TORCHRUN_BIN}" --standalone --nproc_per_node=4 -m lightx2v.infer \
    ...
    --config_json "${lightx2v_path}/configs/hunyuan_image3/hunyuan_image3_ti2i_tp2_sp2_cfg1_serial_flashinfer.json" \
