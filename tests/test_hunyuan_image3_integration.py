...
def test_hunyuan_image3_ti2i_script_passes_cache_args_in_python_entrypoint():
    ...
    python_entry = script_source.split("lightx2v.infer", maxsplit=1)[1]
...
def test_hunyuan_image3_t2i_script_passes_cache_args_in_python_entrypoint():
    ...
    assert 'HUNYUAN_CFG_MODE="${hunyuan_cfg_mode:-parallel}"' in script_source
    assert "torchrun" in script_source
    assert "CFG_PARALLEL_SIZE" in script_source
    for cli_arg in [
        "--moe_impl",
        "--hunyuan_cfg_mode",
        "--attn_impl",
...
def test_hunyuan_image3_cfg_parallel_flashinfer_tune_uses_rank_local_cache(tmp_path, monkeypatch):
    from lightx2v.models.runners.hunyuan_image3 import hunyuan_image3_runner
    from lightx2v.models.runners.hunyuan_image3.hunyuan_image3_runner import HunyuanImage3Runner

    runner = HunyuanImage3Runner.__new__(HunyuanImage3Runner)
    runner.config = {"cfg_parallel": True}
    runner._cfg_parallel_rank = lambda: 1
    monkeypatch.setattr(hunyuan_image3_runner.dist, "is_available", lambda: True)
    monkeypatch.setattr(hunyuan_image3_runner.dist, "is_initialized", lambda: True)

    shared_cache = tmp_path / "flashinfer_cache.json"
    shared_cache.write_text('{"cached": true}')

    resolved = runner._resolve_flashinfer_autotune_cache("tune", str(shared_cache))

    assert resolved == str(tmp_path / "flashinfer_cache.cfg_rank1.json")
    assert Path(resolved).read_text() == '{"cached": true}'
